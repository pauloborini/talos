...
| env `TALOS_HOST=vscode` (injetado pelo instalador `init vscode` em `.vscode/mcp.json` ou no `settings.json` do usuário) | `vscode` |
| env `TALOS_HOST=mavis` (injetado por `servers.mcp.json` do Plugin V1 do MinimaxCode) | `mavis` |
| nenhum | `generic` |

## Matriz de adapters

| Concern | `claude` (Claude Code) | `codex` (Codex App) | `opencode` | `pi` (pi cli) | `antigravity` (Gemini) | `zcode` (ZCode) | `vscode` (VS Code) | `mavis` (MinimaxCode) | `generic` |
|---------|------------------------|---------------------|------------|---------------|------------------------|-----------|-----------|------------------------|-----------|
| Disparo de subagente | `Agent(subagent_type: "<name>", prompt: "<state_path>")` | `spawn_agent(agent_type: "<name>", items: [{ type: "text", text: "<state_path>" }])` | `@<name>` (ou auto) com `<state_path>` | tool `subagent({ agent: "<name>", task: "<state_path>", context: "fresh" })` (pi-subagents) | `define_subagent` + `invoke_subagent` com `<state_path>` | `Agent(subagent_type: "<name>", prompt: "<state_path>")` | `runSubagent(agentName: "<name>", prompt: "<state_path>")` | `task({ agent_name: "talos-<name>", prompt: "<state_path>" })` foreground (síncrono) ou `mavis session send` (síncrono) | subagente nativo do host, passando só `<state_path>` |
| Registro do subagente | `agents/<name>.md` na raiz do plugin | `CODEX_HOME/agents/<name>.toml` via `init codex` (`.codex/agents/` no bundle é fonte gerada; custom agent nativo; `developer_instructions` carrega o `SKILL.md`; sem pin de modelo, herda default suportado pela conta/host) | `.opencode/agents/<name>.md` (`mode: subagent`) | `.pi/agents/<name>.md` (pi-subagents; frontmatter `name`+`description`+`tools`; **`SKILL.md` canônico embutido no corpo** porque o pi não tem skill loader no sub-agente — fonte única segue `packages/skills/<name>/SKILL.md`, agente é cópia gerada por `build/gen-host-agent.mjs`) | dinâmico via `define_subagent` da skill do orquestrador | `agents/<name>.md` na raiz do plugin (.zcode-plugin) — mesmo formato claude (Claude Agent SDK) | `agents/<name>.md` no prompt folder do VS Code (`…/Application Support/Code/User/prompts/` no macOS) ou `.vscode/agents/` | `~/.minimax/agents/talos-<name>/config.yaml` (custom agent MinimaxCode; system_prompt derivado de `agents/<talos-<name>.md`; frontmatter `name`+`description`; tools do subagente preservadas) | mecanismo nativo equivalente |
| Topologia do validador frio (G4) | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** | **`sibling`** |
| Fallback de subagente (limitação do host) | — | — | — | — | — | `subagent_dispatch.fallback.enabled:true` — despacha `general-purpose` (nativo, herda MCP) lendo `agents/<name>.md` como system prompt; sub-agentes de plugin não herdam MCP no ZCode (confirmado v0.10.1) | — | — (custom agents MinimaxCode herdam MCP do Plugin V1) | — |
| Join síncrono (gate JOIN) | `self_evident` (`Agent()` bloqueante) | `self_evident` (confirmado em produção) | `self_evident` (`@<name>` bloqueante) | `must_report` (depende de `pi-subagents`; hard-fail sem report) | `self_evident` (`invoke_subagent` bloqueante) | `self_evident` (`Agent()` bloqueante; Claude Agent SDK) | `self_evident` (`runSubagent` bloqueante; confidence high) | `self_evident` (`task` foreground / `session send` bloqueante por design; confidence high) | `must_report` (indeterminado; hard-fail sem report) |
| Capacidade de mutação (gate DISPATCH, DEC-008) | `mutable` (Write/Edit/Bash verificados em produção) | `mutable` (verificado em produção) | `mutable` (verificado em produção) | `unknown` (depende de `pi-subagents`; exige `dispatch_mutable` no report) | `unknown` (não verificado; exige `dispatch_mutable` no report) | `unknown` (harness pode restringir `subagent_type`; exige `dispatch_mutable` no report) | `mutable` (confirmado em produção) | `mutable` (custom agents MinimaxCode herdam Read/Write/Edit/Bash/Glob/Grep; verificado por smoke) | `unknown` (exige `dispatch_mutable` no report) |
| Todo nativo | `TodoWrite` | `tasks` | `todowrite` | nenhum (segue sem mirror) | nenhum (segue sem mirror) | `TodoWrite` | `manage_todo_list` | `todowrite` | nenhum (segue sem mirror) |
| Entrevista estruturada (`question_prompt`) | `AskUserQuestion` | `request_user_input` (max 3 perguntas) | `question` | `interactive_prompt` | `ask_question` (`resume_after_interview: automatic`) | `AskUserQuestion` | `vscode_askQuestions` | `ask_user` (tool nativa; 1–4 steps, single/multiple) | `native_structured_question` |
| Config MCP | `plugin.json` `mcpServers` | `.mcp.json` | `opencode.json` `mcp.<name>` (`type:"local"`, `environment.TALOS_HOST=opencode`) | `.mcp.json` no root (`pi-mcp-adapter`; `env.TALOS_HOST=pi`; tools chegam proxiadas/prefixadas `talos_<tool>`) | `mcp_config.json` (`env.TALOS_HOST=antigravity`) | `.zcode-plugin/plugin.json` `mcpServers` (stdio; `ZCODE_PLUGIN_ROOT` injetado pelo host) | `.vscode/mcp.json` (projeto) ou `settings.json` `github.copilot.chat.mcpServers` (global; `env.TALOS_HOST=vscode`) | Plugin V1 do MinimaxCode: `.minimax-plugin/plugin.json` + `servers.mcp.json` (stdio; `env.TALOS_HOST=mavis`); instalado em `~/.minimax/plugins/talos/` | host MCP-capaz |
| Deps externas obrigatórias | — | — | — | **`pi-mcp-adapter` + `pi-subagents`** (DEC-005) | — | — | — | — (MinimaxCode nativo; nada além do Plugin V1 instalado) | — |
| Estado de run | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) |
| Escrita de plano | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` |
| Leitura de plano (ordem) | `.talos/plans/` → `.cursor/plans/` → `.codex/plans/` | idem | idem | idem | idem | idem | idem | idem | idem |

`.cursor/plans/` e `.codex/plans/` são lidos com deprecation warning por 1 release; escrita só em `.talos/plans/`. **opencode** instala via `.opencode/` + `opencode.json` (`hosts/opencode/`). **pi** instala via `mcp.json` + `agents/` + `skills/` (`hosts/pi/`) e exige as 2 deps obrigatórias; sem qualquer uma o preflight aborta (gate PREREQ). **vscode** instala via `.vscode/talos/` + `.vscode/mcp.json` (projeto) ou `~/.vscode-talos/` + prompt folder + `settings.json` (global; `hosts/vscode/`).
...
| `question_prompt` | obj | `{mechanism, mode: 'structured', max_questions, options_per_question, persistence, resume_after_interview?}` — verbo nativo de entrevista estruturada usado por `talos-sprint-interview` e `talos-backlog-generator` (rodadas de múltipla escolha; `max_questions`/`options_per_question` vêm do descritor, nunca de constante da skill). Mecanismo por host: `AskUserQuestion` (claude/zcode), `request_user_input` (codex, max 3), `question` (opencode), `interactive_prompt` (pi), `ask_question` (antigravity, com `resume_after_interview: 'automatic'`), `vscode_askQuestions` (vscode), `ask_user` (mavis; 1–4 steps, single/multiple), `native_structured_question` (generic). Ausente/indisponível → a skill bloqueia a rodada (não degrada para pergunta livre). |
...
Todos os hosts-alvo do survey S01 e expansões posteriores estão implementados na matriz acima: `claude`, `codex`, `cursor` (carona no manifest claude), `antigravity`, `zcode` (Claude Agent SDK compat), `opencode` (S06), `pi` (S07), `vscode` (Copilot Chat nativo), `mavis` (MinimaxCode) e `generic`. Nenhum exige HTTP/SSE → stdio único (DEC-006/S05). O survey S01 é histórico; o arquivo `PRD_S01_host_survey.md` não está neste repo — a matriz acima é a referência estática vigente.
