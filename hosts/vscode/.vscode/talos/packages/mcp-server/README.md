...
- `talos_verify_template_conformance`: Gate TC; PLAN só avança com template conforme e `pending_count: 0`; em fluxo de sprint use `require_sprint_file:true` para exigir referências `EVAL-*`/`Eval/Policy` e validar `intent_refs` por task (PLAN ⊆ §2: cada `Tnn` declara SF-* ou R1 da §2 saturada; superfícies/recusa sem lastro bloqueiam).
- `talos_verify_sprint_file`: Gate de sprint viva; valida `SPRINT_S<NN>_*.md`, `eval_manifest`, `policy_manifest`, evidence-to-claim, contrato §7 e vínculo com backlog quando fornecido. Arg opcional `require` (`stub`|`plan_ready`; omitido = `plan_ready`): `stub` exige estrutura base **incluindo** `Intenção status` (`rascunho`|`saturada`) e `Selo da intenção` (`pendente até saturação` ou `sha256:`); `plan_ready` inclui §2 saturada (eixo/SF/AS), selos íntegros e contrato executável. Resposta inclui `require` ecoado e `maturity` (`stub`|`plan_ready`). Sem `legacy_sealed`.
...
- `talos_select_next_sprint`: Gate de seleção. A fila de maturação (`state=backlog`, sprint file stub válido, DoR amarelo/verde) **sempre** precede qualquer `ready`, com ou sem `loop`. Depois, escolhe sprint executável com `state=ready`, deps internas `done` **ou** `manual_validation_pending`, sprint file válido e DoR verde. O arg estritamente booleano `loop:true` ativa a esteira de execução serial do orquestrador (`--loop`, CN7): re-seleciona em cada ciclo/retomada; **não** é o único opt-in de maturação. Ausente ou `false` não limita a seleção só a `ready`. Nunca abre exceção para `blocked`, `detached_repair`, DoR vermelho/ausente ou dependência pendente. Arg opcional `mode` (`full`/`direct`/`execute`/`interview-only`). Em `passed`, `next_action` é mode-aware: `maturity !== plan_ready` ou §7 draft → `sprint_interview`; `direct` + `plan_ready` → `plan_execute`; `full` + `plan_ready` sem PLAN → `plan_handoff`; PLAN real → `plan_execute`. `selected` ecoa `maturity`, `intencao_status`, `intencao_sealed`. Campo `selected.prd_path` é legado posicional do backlog (null/`—` esperado).
...
- `talos_scan_acceptance`: Gate G5; escaneia o contrato §7 do sprint file por padrões determinísticos de ambiguidade bloqueante. `blocking_count: 0` **não** pula L2: `next_action` é `consultar_maturidade` (o orquestrador segue `talos_verify_sprint_file.maturity`).
