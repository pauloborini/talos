...
Se faltar informação não bloqueante, gere o backlog com premissas marcadas e registre perguntas/riscos. O gatilho L1 de pergunta é **ambiguidade de recorte/tema/MoSCoW do ciclo** (passo 4), não `talos_scan_acceptance` nem lacunas de `behavior` em AC YAML — stubs nascem com §7 mínima/vazia de propósito. **Proibido perguntar quantas sprints**; decomposição é efeito do escopo, não input de entrevista.
...
4. **Fechar ambiguidade de recorte (L1) por entrevista estruturada:** antes de gravar qualquer artefato, monte o rascunho em memória (`sprint_markdown`) e escane com `talos_scan_acceptance(sprint_markdown: <rascunho>)` para aferir ambiguidade de **recorte do ciclo** — o que entra/fica fora, MoSCoW dos **temas**, sequência só se um tema bloqueia outro. **Não** use `talos_scan_acceptance` como gatilho único de pergunta nem inflar §7/AC YAML «para parecer completo» enquanto `blocking_count > 0`. Enquanto o recorte/tema/MoSCoW estiver ambíguo: (a) chame `talos_capabilities` e leia `question_prompt` — `max_questions` / `options_per_question` vêm do descritor do host, nunca de constante da skill; sem `question_prompt` → **bloqueie a rodada** (não degrade para pergunta livre); (b) conduza rodadas sobre **temas e limites do ciclo**, não sobre `behavior` de AC, loading/vazio, componente ou path HTTP — catálogo do inútil L1 (spec §5.4); (c) stem cita o tema do ciclo com substantivo do recorte, nunca «em geral»; (d) aplique respostas com `Origem: usuario` ao fim de cada rodada — em memória antes de gravar, ou `persistInterviewRound` quando o sprint file já existir em disco; (e) reindexe o rascunho e recalcule lacunas de recorte. Se o usuário **sugerir** N sprints, nomes ou IDs de sprint, grave `Origem: usuario` e **não sobrescreva** — silêncio → LLM decompõe. Decisão não fechada vira premissa registrada, não pergunta repetida.
...
6. **Criar/atualizar sprint files (stub L1):** para cada sprint nova ou alterada, preencha `SPRINT_TEMPLATE.md` em `.talos/backlog/sprints/SPRINT_S<NN>_<slug>.md` (ou path pedido). Stubs nascem com `Status: backlog`, §1 **`Intenção status: rascunho`** e **`Selo da intenção: pendente até saturação`** (linhas obrigatórias — placeholder `[rascunho / saturada]` não passa `require: stub`), `Contrato status: draft`, **`Status DoR: amarelo`** (token único — não deixar o placeholder `[verde / amarelo / vermelho]`), §2 mínima (objetivo/valor — eixo/SF/AS/R1 maturam em L2), **§7 sem YAML `acceptance` obrigatório** (D*/UX/AC mínimos ou vazios — não inflar AC fantasma), mais `eval_manifest`, `policy_manifest` e evidence-to-claim quando aplicável.
...
- **Proibido perguntar quantas sprints** ou pedir estimativa de quebra S01a/tasks — decomposição é efeito do escopo; sugestão do usuário prevalece com `Origem: usuario`.
