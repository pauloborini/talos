...
| SELECT_NEXT_SPRINT | **Próxima sprint vem do MCP.** Em `backlog_first`, chamar `talos_select_next_sprint`; sem `selected` não há avanço ao plano. A seleção exige `state=ready`, deps internas `done`, sprint file válido e DoR verde. | roteamento |
