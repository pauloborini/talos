...
| Join síncrono (gate JOIN) | `self_evident` (`Agent()` bloqueante) | `self_evident` (confirmado em produção) | `self_evident` (`@<name>` bloqueante) | `must_report` (depende de `pi-subagents`; hard-fail sem report) | `self_evident` (`invoke_subagent` bloqueante) | `self_evident` (`Agent()` bloqueante; Claude Agent SDK) | `self_evident` (`runSubagent` bloqueante; confidence high) | `must_report` (indeterminado; hard-fail sem report) |
| Capacidade de mutação (gate DISPATCH, DEC-008) | `mutable` (Write/Edit/Bash verificados em produção) | `mutable` (verificado em produção) | `mutable` (verificado em produção) | `unknown` (depende de `pi-subagents`; exige `dispatch_mutable` no report) | `unknown` (não verificado; exige `dispatch_mutable` no report) | `unknown` (harness pode restringir `subagent_type`; exige `dispatch_mutable` no report) | `mutable` (confirmado em produção) | `unknown` (exige `dispatch_mutable` no report) |
| Todo nativo | `TodoWrite` | `tasks` | `todowrite` | nenhum (segue sem mirror) | nenhum (segue sem mirror) | `TodoWrite` | `manage_todo_list` | nenhum (segue sem mirror) |
| Entrevista estruturada (`question_prompt`) | `AskUserQuestion` | `request_user_input` (max 3 perguntas) | `question` | `interactive_prompt` | `ask_question` (`resume_after_interview: automatic`) | `AskUserQuestion` | `vscode_askQuestions` | `native_structured_question` |
| Config MCP | `plugin.json` `mcpServers` | `.mcp.json` | `opencode.json` `mcp.<name>` (`type:"local"`, `environment.TALOS_HOST=opencode`) | `.mcp.json` no root (`pi-mcp-adapter`; `env.TALOS_HOST=pi`; tools chegam proxiadas/prefixadas `talos_<tool>`) | `mcp_config.json` (`env.TALOS_HOST=antigravity`) | `.zcode-plugin/plugin.json` `mcpServers` (stdio; `ZCODE_PLUGIN_ROOT` injetado pelo host) | `.vscode/mcp.json` (projeto) ou `settings.json` `github.copilot.chat.mcpServers` (global; `env.TALOS_HOST=vscode`) | host MCP-capaz |
| Deps externas obrigatórias | — | — | — | **`pi-mcp-adapter` + `pi-subagents`** (DEC-005) | — | — | — | — |
| Estado de run | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) | `talos_run_state` (MCP) |
| Escrita de plano | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` | `.talos/plans/` |
| Leitura de plano (ordem) | `.talos/plans/` → `.cursor/plans/` → `.codex/plans/` | idem | idem | idem | idem | idem | idem | idem |
