...
| SELECT_NEXT_SPRINT | **Próxima sprint + verbo vêm do MCP.** Em `backlog_first`, chamar `talos_select_next_sprint` com `mode` do pipeline; em `--loop`, também `loop:true` em cada seleção/reinício. Sem `selected` não há avanço. A seleção normal exige `state=ready`, deps internas `done`/`manual_validation_pending`, sprint file válido e DoR verde. Só `loop:true` admite a pré-etapa `state=backlog` com DoR amarelo/verde e devolve obrigatoriamente `sprint_interview`; após a entrevista, re-selecionar antes de plano/execução. Em `passed`, **obrigatório** seguir `next_action` ∈ {`sprint_interview`,`plan_handoff`,`plan_execute`} (mode-aware: `direct` nunca sugere `plan_handoff`; §7 draft → interview). | roteamento |
...
1. **Seleção** — `talos_select_next_sprint({mode, loop:true})` em **cada** ciclo e retomada. A flag é exclusiva do `--loop`; sem ela o fluxo normal não muda. O MCP prioriza estritamente uma candidata `backlog` maturável antes de qualquer `ready`; ela vem com `next_action:sprint_interview`: executar a entrevista, persistir/sela a §7 e **voltar a esta seleção** antes de plano ou execução. Só quando não houver backlog maturável aplica o ranking normal das `ready`. `status: blocked` sem `selected` → **relatório final** e fim da campanha.
...
6. **Avanço automático (D14)** — terminada a cadeia da sprint (fechamento OU estacionamento, residual P2/P3 registrado), voltar ao passo 1 **sem pedir permissão entre gates**. A entrevista do contrato §7 é a **única pausa in-band** da esteira; todo o resto é "continuação automática" (Princípio de continuação automática). Após veredito terminal do validator, chamar `talos_update_sprint_status` no mesmo turno e obrigatoriamente antes do `talos_select_next_sprint` seguinte (D17); se o MCP devolver `next_action: reconcile_state`, não avançar sprint.
