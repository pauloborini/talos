---
name: talos
description: "Orquestra pipeline completo de desenvolvimento de features: /talos <mode> <input-type> [flags]. Automatiza backlog macro → sprint file (intenção §2 saturada + contrato §7) → entrevista L2 dual (se necessário) → planejamento → execução → review (opcional) e oferece audit universal sem correção. Pipeline orientado a artefato com gates duros: cada fase só conta se produzir arquivo verificável em disco."
...
- **`full`** — pipeline completo: backlog macro (se necessário) → sprint file com intenção §2 saturada + contrato §7 → validação/entrevista L2 dual (se necessário) → **plano (artefato obrigatório)** → executor → review (opcional)
- **`direct`** — pipeline enxuto: backlog macro (se necessário) → sprint file com intenção §2 saturada + contrato §7 → validação/entrevista L2 dual (se necessário) → `talos-direct-execute` → review (opcional). **Não produz plano de handoff** — a diferença real para `full` é exatamente essa.
...
- **`interview-only`** — entrevista direta (ex: brainstorm, resolução de decisões). Entrevista **sem execução**: não usa `guarantee_level` no fluxo (não há execução de código a garantir). Permanece modo separado. Saída = sprint file standalone com intenção §2 saturada+selo e contrato §7 (não PRD).
...
- `--interview` — força entrevista L2 dual (**intenção §2 + contrato §7** do sprint file) mesmo sem ambiguidades detectadas na §7
...
→ Resolve S05 no backlog, valida sprint file (intenção §2 + contrato §7), entrevista L2 se necessário, cria PLAN_*.md, executa a partir do plano

/talos direct sprint "S05"
→ Resolve S05 no backlog, valida sprint file (intenção §2 + contrato §7), executa direto sem PLAN_*.md

/talos full idea "melhorar performance de listagem" --interview
→ Prioriza BACKLOG_MESTRE_*.md quando a entrada ainda é macro, cria/atualiza sprint file, seleciona a próxima sprint executável, força entrevista L2 dual (§2+§7), plano, executor

/talos interview-only brainstorm "que tal dark mode?"
→ Cria sprint file standalone mínimo pelo SPRINT_TEMPLATE.md (§2 rascunho + §7 draft), valida o path e entrevista via talos-sprint-interview (fase 1 §2 → selo intenção; fase 2 §7 → selo contrato); sem execução
...
3b. **Gate DEP — dependência de backlog (só `sprint`/`backlog-item`).** Se o item declara `Dependências` no backlog/registro de origem, ler o status de cada dependência **no mesmo backlog**. Se alguma não estiver `done` ou `manual_validation_pending` (D5: MVP satisfaz DEP), **abortar em `ready`** com `unmet_dependencies`, causa e `next_action` — determinístico, sem pergunta. Todas `done`/`manual_validation_pending` (ou sem dependências) → segue. Decisão em aberto **não** entra aqui (não é dependência de execução).
   ...
   ```
3c. **Gate Backlog/Sprint — índice e recorte vivo obrigatórios (`full`/`direct` com `sprint`, `backlog-item` ou `backlog_first`).** Quando houver backlog, chamar `talos_verify_backlog_index`; para macro input, chamar também `talos_select_next_sprint` **com `mode` do pipeline atual** e usar somente o `selected` + `next_action` retornados pelo MCP — **ecoar `next_action` no ledger**. Com `--loop`, passar adicionalmente `loop:true` (boolean estrito) em **toda** seleção, inclusive na retomada: ativa a esteira de execução serial além da maturação. **Sem `--loop`**, a fila de maturação (`state=backlog`, sprint file válido no limiar **`stub`**, deps satisfeitas, DoR amarelo/verde) **ainda existe** e precede as `ready` — `next_action: sprint_interview` (DEC-048); `--loop` não é o único jeito de entrevistar stub L1. Resolver o `Sprint file` linkado/selecionado e chamar `talos_verify_sprint_file` (**`require: stub`** para generator/select em maturação; **`plan_ready`** default antes de plano/direct). Se backlog inválido, seleção ausente, sprint file `pendente`, inexistente, divergente do Sprint ID, sem seções mínimas ou gate bloqueado/indisponível, **abortar antes do plano** com causa e `next_action`. **Após `passed`, o `next_action` é obrigatório e mode-aware:** `sprint_interview` → maturar §2+§7 e **reselecionar antes de plano/execução**; `plan_handoff` → redigir PLAN (só `full`/`execute`); `plan_execute` → despachar executor do modo. Proibido ignorar o verbo MCP ou inventar `gerar_prd`/PRD.
   ```text
...
- Parar por decisão em aberto. Decisão pendente de **qualquer fonte** (scan de aceite, entrevista, `PERGUNTAS_EM_ABERTO.md`, doc de discussão/decisões como `DISCUSSAO_*.md`, ou o próprio backlog) **não é blockage**: garante sprint file com intenção §2 **e** contrato §7, dispara `talos-sprint-interview` (L2 dual) sobre ele, propaga e **continua**. Nunca oferecer "responda só: seguir com recomendação ou D=...". Ver "Decisão em aberto ≠ parada".

**Contrato §7 / intenção §2 incompletos em `full`/`direct`** = a fase de produto **matura intenção §2 e contrato §7 no sprint file** (invoca `talos-sprint-interview` / autoria documental no fio principal). Nunca perguntar "quer que eu gere?".

**Backlog ausente em macro input (`routing.document_flow.priority = backlog_first`)** = antes do plano, invocar `talos-backlog-generator` no fio principal para criar/atualizar `BACKLOG_MESTRE_*.md` e `SPRINT_S<NN>_*.md`, validar artefatos em disco, chamar `talos_verify_backlog_index` e escolher a próxima sprint executável via `talos_select_next_sprint`. Em seguida, maturar intenção §2 **e** contrato §7 somente a partir do sprint file dessa sprint. Isto preserva o escopo pequeno de execução: macro fica no backlog mestre; sprint file fecha recorte vivo + intenção + contrato; plano/executor recebem apenas a sprint selecionada.
...
- **ANTES do plano validado — autoria documental livre no fio principal.** Pode maturar intenção §2 e contrato §7 no sprint file, entrevistar e escrever `PLAN_*.md` direto; fases documentais não exigem sub-agent (documento não muta o produto). **Ao saturar intenção §2, gravar `Intenção status: saturada` + `Selo da intenção: sha256:<hash>`**; **ao aprovar o contrato §7, gravar `Contrato status: aprovado` + `Selo do contrato: sha256:<hash>`** — sem selos íntegros o sprint file não avança como L2 pronto.
...
| G5 | **Scan de ambiguidade determinístico e logado — não substitui L2.** `talos_scan_acceptance` continua obrigatório para §7; **zero padrões não pula** saturação de intenção (§2). Despachar `talos-sprint-interview` se `maturity !== plan_ready` **ou** `--interview` **ou** G5>0 (≥1 padrão bloqueante). Não existe "pular porque scan=0". `--interview` sempre força as duas fases L2. | validação intenção §2 + contrato §7 |
...
2. **Contrato §7 (produto)** — garantir intenção §2 saturada **e** contrato §7 (D*, cenários UX, aceite atômico `AC-*`). Chamar `talos_verify_sprint_file` (default `plan_ready`) e `talos_scan_acceptance`.
3. **Interview (condicional)** — invocar `talos-sprint-interview` se **`maturity !== plan_ready`** (ler retorno de `talos_verify_sprint_file`) **ou** `--interview` **ou** `talos_scan_acceptance` com ≥1 padrão bloqueante (G5>0). **Proibido** pular L2 só porque scan=0. Depois reexecutar `talos_verify_sprint_file`, `talos_scan_acceptance` e selos (intenção + contrato).
...
Artefatos esperados: `SPRINT_S<NN>_*.md` (intenção §2 saturada+selo + contrato §7) → (atualizado) → diff de código → relatório do validador. **Sem `PLAN_*.md`** — por design.

1. Parse / Backlog first (condicional) / Sprint file. Se `routing.document_flow.priority = backlog_first`, gerar/atualizar `BACKLOG_MESTRE_*.md` + sprint file antes, chamar `talos_select_next_sprint(mode=direct)` e recortar a execução para `selected`. **Seguir `next_action`:** `sprint_interview` → passo 2; `plan_execute` → passo 4 (nunca `plan_handoff` em `direct`). Intenção e contrato nascem/maturam no sprint file, não no backlog macro.
2. **L2 intenção §2 + contrato §7** — garantir intenção §2 saturada **e** contrato §7 (D*, cenários UX, aceite atômico `AC-*`). Chamar `talos_verify_sprint_file` (default `plan_ready`) e `talos_scan_acceptance`.
3. **Interview (condicional)** — invocar `talos-sprint-interview` (fase 1 §2 + fase 2 §7) se **`maturity !== plan_ready`** (ler retorno de `talos_verify_sprint_file`) **ou** `--interview` **ou** `talos_scan_acceptance` com ≥1 padrão bloqueante (G5>0). **Proibido** pular L2 só porque scan=0. Depois reexecutar `talos_verify_sprint_file`, `talos_scan_acceptance` e selos (intenção + contrato).
4. **Execute** — após `talos_verify_sprint_file` em **`plan_ready`**, rodar o passo **[EXEC]** (executor lê o sprint file / intenção §2 + contrato §7; sem `PLAN_*.md`).
5. Review (obrigatória sob policy; condicional senão) — só após executor retornar 100% e dispatch MCP permitir; `policy_manifest.critical_review.required: true` no sprint file torna a slice-review obrigatória mesmo sem `--review` (G8).
6. Output (ledger verificado).
...
1. Se a entrada já for sprint file válido com intenção §2 e §7, usar seu path. Se for `brainstorm`, criar primeiro um draft mínimo em disco com `packages/templates/SPRINT_TEMPLATE.md`: `Backlog mestre: Não aplicável (standalone)`, §2 em rascunho (objetivo/eixo/SF/AS/R1) e §7 em draft (D* / cenários UX / aceite), registrando o brainstorm em objetivo/contexto. Nunca criar PRD draft.
...
3. Invocar `talos-sprint-interview` no fio principal com `sprint_file_path` válido (duas fases: saturar §2 + selar intenção, depois maturar §7 + selar contrato); persistir respostas no sprint file; reverificar.
...
Ativação: flag `--loop` em `full`/`direct` com backlog (macro input ou `sprint`/`backlog-item`). Só existe em pipeline com execução; `audit`/`interview-only` não têm loop. **Sem `--loop`, esta seção (esteira serial, drain, sidecar, review sempre) não se aplica** (CN7, opt-in). A fila de maturação L2 em `talos_select_next_sprint` **não depende** desta flag (DEC-048); a cadeia de fechamento da review comum aos dois modos já está no [EXEC].

**Início da sprint:** o `talos_lock_dispatch(action=start, phase=plan_execute)` de **cada sprint** da esteira leva `options:{loop:true}` (boolean estrito; valor não-booleano é recusa `-32602`). A flag é gravada no ledger daquele run (`data.options.loop`) pelo MCP — e é do MESMO run que o `talos_update_sprint_status` fecha a sprint, onde o gate de `--loop` recusa `done`/`manual_validation_pending` sem o gate `slice_review` `passed` (D12/VC3). Sem a flag, `options.loop` não é gravado e os gates D12/VC3 de review-sempre não se aplicam.

Ciclo serial — uma sprint por vez, nesta ordem, nada em paralelo:

1. **Seleção** — `talos_select_next_sprint({mode, loop:true})` em **cada** ciclo e retomada. `loop:true` é exclusivo da esteira `--loop`. O MCP prioriza estritamente uma candidata `backlog` maturável antes de qualquer `ready` **também sem** `--loop` (DEC-048); ela vem com `next_action:sprint_interview`: executar entrevista L2 dual (§2+§7), persistir/selar intenção + contrato §7 e **voltar a esta seleção** antes de plano ou execução. Só quando não houver backlog maturável aplica o ranking normal das `ready`. `status: blocked` sem `selected` → **relatório final** e fim da campanha. Se `next_action=drain_pendencies` (ou `drain_required.required` + `advance_blocked`), **não** tratar `selected` como avanço — ir ao passo 2.
2. **Drain sob demanda (CN4/D9/D20)** — se o retorno trouxer `next_action: drain_pendencies` ou `drain_required.required: true` (reasons: `threshold` — teto de 3 PDs abertas; `dep_cone` — PD cuja sprint de origem está no cone DEP do candidato; `files_overlap` — overlap entre `files` da PD e os paths do §3 do sprint file do candidato; `pd_ids[]` = PDs a fechar; `threshold_crossed` quando `open_pd_count` ≥ 3), drenar **antes de avançar**: despachar o sidecar `talos-escalation-repair` em **modo drain** (blocking, um por vez — G9) para as PDs em `pd_ids`; ao `repair_complete` com `origin=escalation` o MCP devolve `next_action: close_pendencies_and_reselect` (**nunca** `dispatch_task_validator_retry` — G8). **Guard legado:** se `repair_complete` ainda pedir `dispatch_task_validator_retry` **e** `origin=escalation` **e** a fase ativa **não** for `plan_execute` da mesma slice, **ignorar o retry** — fechar PDs, re-selecionar, ecoar `repair_complete_vs_g8` no ledger. Fechar cada PD com `talos_pendencies(action=close, {pd_id})` — a escrita do arquivo é sempre do MCP; o orquestrador **nunca** grava em `PENDENCIAS_<slug>.md`. Depois re-chamar `talos_select_next_sprint`. O drain é acionado **somente pelo retorno do MCP** — não existe faxina obrigatória de todas as PDs antes de qualquer sprint (D9).
...
4. **Residual P2/P3 + fechamento** — checklist pós-`slice_review` verde: (a) para cada finding não-blocking, `talos_pendencies(action=append, {sprint_id, severity, files, recommendation, fix_validation})` — o MCP gera `PD-<sprint>-<NN>` monotônico; (b) `talos_update_sprint_status` terminal (`done` ou `manual_validation_pending`) **sem hops** `ready`/`doing`/`review`; (c) **só então** `talos_select_next_sprint`; (d) se drain, passo 2 e `select_next` de novo — sem interview/plan da `selected` informativa.
...
6. **Avanço automático (D14)** — terminada a cadeia da sprint (fechamento OU estacionamento, residual P2/P3 registrado), voltar ao passo 1 **sem pedir permissão entre gates**. A entrevista L2 dual (intenção §2 + contrato §7) é a **única pausa in-band** da esteira; todo o resto é "continuação automática" (Princípio de continuação automática). Após veredito terminal do validator, chamar `talos_update_sprint_status` no mesmo turno e obrigatoriamente antes do `talos_select_next_sprint` seguinte (D17); se o MCP devolver `next_action: reconcile_state`, não avançar sprint.

**Few-shot S03→S04 (threshold + dep_cone):** `slice_review` `pass_with_observations` da S03 → `pendencies append` `PD-S03-01..03` → `update_sprint_status(done|manual_validation_pending)` **sem hops** `ready`/`doing`/`review` → `select_next` devolve `selected=S04` **informativo** + `next_action=drain_pendencies` + `advance_blocked` + `pd_ids` (banner cita teto se `open_pd_count≥3`). **Não** `lock_dispatch(plan_handoff|plan_execute)` da S04 (MCP: `expected_phase=drain_pendencies`). Sidecar modo drain → `repair_complete(origin=escalation)` → `close_pendencies_and_reselect` → `pendencies close` ×3 → `select_next` de novo (S04 avançável). O `lock_dispatch(start)` da S04 no mesmo `run_id` zera `validator_cycle`/`gates.slice_review` da S03 — G4 da S04 abre; `repair_closed` só bloqueia retry na **mesma** slice.
...
O scan é **determinístico** e roda **dentro do MCP** (`talos_scan_acceptance`): o orquestrador **não** reaplica padrões por conta própria, só consome o resultado.

**Threshold = 1 para §7.** Se ≥ 1 padrão bloqueante → entrevista fase 2 (§7) dentro de `talos-sprint-interview`. **Gate G5 não pula L2:** mesmo com **zero** padrões, se `maturity !== plan_ready` ou `--interview`, despachar entrevista (fase 1 §2 obrigatória). Registrar no ledger: scan executado + decisão de entrevista (motivo: maturidade / flag / padrões) — **nunca** declarar skip de L2 só porque scan=0.
...
1. **Garantir sprint file com intenção §2 e contrato §7 primeiro.** Em `full`/`direct`, se intenção ou contrato não existem/estão draft incompletos, validar/criar primeiro o sprint file (quando o fluxo exigir sprint), então **maturar §2 e §7**. A entrevista é **sprint-scoped**: roda **sobre** o sprint file (L2 dual), nunca antes dele existir.
...
3. **Persistir após cada rodada** no mesmo sprint file (§2 + §7), reindexar e não repetir decisões fechadas; ao saturar intenção, gravar selo §2; ao aprovar contrato, gravar selo §7.
...
📄 Sprint: /path/to/SPRINT_S05_login.md   [intenção §2 saturada+selo · contrato §7 aprovado+selo]
...
Status:
  ...
  ✅ Sprint file: passed [MCP: talos_verify_sprint_file — intenção §2 + contrato §7]
  ✅ Aceite scan: 0 padrões; entrevista L2 por maturidade §2 [MCP: talos_scan_acceptance + talos_verify_sprint_file / G5]
...
| `talos-sprint-interview` | sprint_file_path, ambiguities | `SPRINT_*.md` intenção §2 saturada+selo + §7 atualizado + selo contrato ao aprovar |
