...
description: Executa planos produzidos por `talos-plan-handoff` task-a-task com gates finitos e self-repair local pré-handoff (lint/tests/diff — máximo 2 passes por task). Ao concluir todas as tasks da slice, chama `talos_commit_state` com julgamento curto (`proofs[]`), para toda mutação e retorna `validator_handoff_required` com o `state_path` do retorno ao orquestrador para validação fria sibling via `talos-task-validator`. O orquestrador consome o veredito e decide sobre repair ou fechamento — este executor nunca valida o próprio trabalho nem processa resultado do validador. Use quando o cliente precisar implementar um plano sem derivar dos invariantes.
---
