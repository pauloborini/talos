...
- `worktree_baseline` é capturado pelo MCP em `talos_lock_dispatch(action=start, phase=plan_execute)` (t0). `first_write` permanece no G12 público somente como heartbeat de liveness e não redefine t0 (D6/D21). `worktree_final` é projetado pelo MCP no commit contendo apenas os paths de `files_changed` (JSON magro, D16). Ambos usam tuplas únicas/ordenadas `[path,status,sha256]`; `status` é `A|M|D|R|C|T|U`, delete usa `sha256:null`, symlink usa SHA-256 do target textual.
- `base_sha` e `head_sha` são commits explícitos; não inferir base pelo nome da branch. `HEAD` atual pode avançar se `head_sha` for ancestral e `git diff head_sha HEAD` nos paths da slice for vazio (commit paralelo fora da fatia). Drift ou rewrite nesses paths, ou `head_sha` fora da história, invalida o boundary. `files_changed` é fato git: `(porcelain agora Δ t0) ∪ diff(base_sha…HEAD)` minus `.talos/` (D7/D22). Dirty preexistente byte/status-idêntico em t0 e now fica fora; WIP paralelo pós-t0 entra no fact. Prova é subconjunto do fato (`proofs.files ⊆ files_changed`, D9): claim fora do fact é recusado com `blocked` no commit.
