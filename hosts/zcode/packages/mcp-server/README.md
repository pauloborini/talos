...
Servidor MCP do plugin Talos v0.12.1.
...
- State de sprint: quando `.talos/state/<run_id>/<slice>.json` declara `sprint_file_path`, o boundary exige `eval_results`, `evidence_to_claim` e `policy_scope`; todo `EVAL-*` do sprint file precisa estar `passed` com evidência, e `policy_scope.forbidden_scope` bloqueia arquivo tocado.
