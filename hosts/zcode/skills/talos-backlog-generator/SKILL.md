...
- fase `backlog_first` do `talos`, quando macro input ainda não tem backlog canônico e precisa ser decomposto antes de PRD/plano/execução.
...
O orquestrador deve passar `sprint_id` + `sprint_file_path` ao `talos-sprint-prd-generator`. Macro fica no backlog mestre; PRD/plano/executor recebem apenas a sprint selecionada.
...
- Ser específico o bastante para orientar sprint files; o PRD de sprint deve nascer do sprint file com `talos-sprint-prd-generator`.
...
- Não gerar PRD/plano/código. Em `backlog_first`, sua saída é backlog + sprint file + próxima sprint executável; a próxima fase pertence ao `talos-sprint-prd-generator`.
