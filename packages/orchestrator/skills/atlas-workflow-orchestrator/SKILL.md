...
| G4 | **Validador frio é sempre sub-agent irmão (sibling), em todos os hosts.** O executor escreve `state_path` e para (retorna `validator_handoff_required`); o orquestrador despacha `task_validator` como sub-agent irmão isolado, trava `atlas_lock_validator` e, em caso de `fail`, roda `repair_start` + despacha **`atlas-findings-repair`** (não o executor completo) antes do **2º e último** validator. `validator_run_id`/`repair_run_id` são obrigatórios para fechar cada slot ativo. O executor nunca valida o próprio trabalho nem despacha o validador no mesmo contexto. **Não existe topologia `nested`** — host sem join síncrono é rejeitado no preflight (gate JOIN). | execução |
