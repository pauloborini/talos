# Guia — Melhorias incrementais das skills Atlas
...
| **Executor** | `atlas-plan-execute` ou implementação bounded equivalente |
| **Validador frio** | `atlas-task-validator` |
| **Review opcional** | `atlas-slice-review` |
...
```text
Leia AGENTS.md e reports/GUIA_MELHORIAS_INCREMENTAIS_SKILLS_ATLAS.md.
...
- `atlas-direct-execute` permite self-check local quando subagente não existe, contrariando PREREQ hard-fail.
- `atlas-plan-execute` declara que não observa o validator, mas pede `validator outcome` no próprio relatório.
- `interview-only` invoca `atlas-prd-interview` sem PRD, enquanto a skill exige PRD existente.
...
- **Objetivo:** alinhar `atlas-direct-execute` ao gate PREREQ.
- **Mudança esperada:** indisponibilidade de subagente/MCP retorna `blocked`; nunca self-check como substituto do validator.
- **Arquivos prováveis:** `packages/skills/atlas-direct-execute/SKILL.md` e mirrors gerados.
...
- **Objetivo:** impedir que `atlas-plan-execute` reporte veredito que não recebe.
...
- **Arquivos prováveis:** `packages/skills/atlas-plan-execute/SKILL.md`, `atlas-direct-execute/SKILL.md` e orquestrador.
...
- **Mudança recomendada:** quando input for brainstorm sem PRD, o orquestrador cria draft mínimo pelo template e só então invoca `atlas-prd-interview` sobre esse artefato.
- **Não fazer:** transformar `atlas-prd-interview` em brainstorming genérico ou pular template.
...
- `atlas-direct-execute` afirma que obrigações/probes estão no state, porém o schema não possui esses campos.
...
- **Regra:** extensão direct obrigatória quando `executor_skill=atlas-direct-execute`; plano continua referenciável por `plan_path`.
- **Não fazer:** remover campos atuais ou alterar capabilities schema v5.

#### T02. Produzir state completo nos dois executores

- **Objetivo:** sincronizar `atlas-plan-execute` e `atlas-direct-execute`.
...
- **Arquivos principais:** `atlas-task-validator`, `atlas-plan-handoff`, references compartilhadas e testes.
...
- **Invariante:** `atlas-backlog-generator` permanece explicit-only e fora da cadeia automática.
...
- [ ] `direct` despacha `atlas-direct-execute`.
