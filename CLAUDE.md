...
- Versão: `0.7.2`. Topologia **sibling-only** (v0.7.0, BREAKING de contrato `atlas_capabilities` schema v3→v5, sem mudança de comportamento de execução): validador frio é sempre sub-agent irmão; executor escreve `state_path` e para; orquestrador despacha `atlas-task-validator`. v0.7.1 = patch de confiabilidade (merge no `upsertState`, version-conflict só em lock real, banner por `artifact_kind`, G4 endurecido R17/R19). v0.7.2 = patch de confiabilidade (sem breaking, schema v5 intacto): `ping().capabilities` derivado de `toolsList()` (fim do drift que omitia `atlas_classify_input`), CI cross-OS (Windows/macOS, node puro), nota de proveniência do token no validator SKILL, `.gitattributes` para artefatos gerados. Smoke S18 multi-host: Claude Code/Codex/Cursor/opencode PASS em tarefa real (2026-06-14); pi pendente.
