...
Tipo: **runtime** + **packaging**. **Sem breaking**. Schema MCP: v5 (inalterado). Disco: v3 (inalterado).

Resumo: destravar o handoff `--loop` S03→S04 (drain congela o FSM; salto terminal sem hops; reset de G4/`slice_review` no mesmo `run_id`) e incluir `references/` das skills no bundle instalado.

Mudanças:
- **`packages/mcp-server/server.js`** — `drain_required` grava `dispatch.next_phase=drain_pendencies`; `lock_dispatch(start)` recusa o ciclo seguinte até `select_next` sem drain; salto `backlog→done|manual_validation_pending` com pipeline completo (review no ledger só se `--loop` ou `critical_review`); `repair_closed` só bloqueia retry na mesma `state_path`; `lock_dispatch(start)` de fase de entrada reseta `validator_cycle` e limpa `gates.slice_review`/`review_cycle`.
- **Skills** — orquestrador: few-shot S03→S04 + guard legado de retry; `talos-escalation-repair` modo drain com `{pd_ids_fixed[], do_not_request_validator_retry: true}`.
- **Packaging** — `.gitignore` ignora só `/references/` na raiz; Plugin V1 copia o diretório da skill (não só `SKILL.md`); guard `skill-refs` falha se SKILL.md citar path ausente. `.npmignore` exclui `.app-work/`, `/references/` e `.git.backup-prefilterrepo/` do tarball npx.
- **Anti-drift** — testes `mode=full`+`plan_handoff` sob drain e fail-open de `slice_review` cross-sprint; `guardHandoffLoop`.
- **Bump `0.23.1`** via `build/bump-version.mjs`.

Impacto:
- `--loop` com PDs abertas não inicia `plan_handoff`/`plan_execute` da sprint seguinte até o drain.
- Sprint seguinte no mesmo `run_id` abre G4; `repair_closed` da anterior não gruda.
- Instalação from-source/npx leva `COLD_BACKLOG_REVIEW_PROMPT.md` e `_shared/references/stack-profiles.md`.

Arquivos/artefatos:
- `packages/mcp-server/server.js`, `packages/mcp-server/server.test.js`
- Skills: orquestrador `talos`, `talos-escalation-repair`
- `build/loop-guard.mjs`, `build/skill-refs-guard.mjs`, `build/cli/talos-init.mjs`, `build/install-host.sh`, `.npmignore`
- Bundles `plugins/` e `hosts/` regenerados
...
Mudanças:
- **`packages/mcp-server/traceability.mjs` (novo, Planos 01–03)** — ledger `.talos/traceability/<slug>.json` (escrita absoluta tmp+rename mode 0o600; fallback de root idêntico ao `consumerRoot` do server — sem divergência entre onde a tool grava e onde o gate lê); `upsert` insert-or-update com validação de `sources[]`/`disposition` (`external` exige `ref`; `deferred`/`rejected` com motivo; `deferred_target` tipado); `verify` cruza `source_refs` do §7.3 via `checkTraceabilityGraph` (função pura compartilhada com o conformance) e **expõe `pilot_metrics`** (leitura read-only das métricas, R5); `receipt` (projeção read-only com **escopo da sprint atribuída** — rows/exceptions/blockers cobrem só REQs com source `sprint:<id>`, mesma regra do gate `done`; rejeita claim do caller; marcadores inconsistentes em qualquer sentido lançam -32002; sprint legacy de backlog sem marca naquele id segue happy); `record_metric` (append de observação de piloto no documento completo; `calls` obrigatório; `coverage` 0–1 ou fração). Projeção REQ→AC única (`traceabilityRequirementRows`) compartilhada por receipt e gate done.
- **`packages/mcp-server/server.js`** — registro da tool `talos_traceability` (actions `upsert|verify|receipt|record_metric`, `sprint_path`/`sprint_id`/`state_path`/`metric`); ramo v1 no gate `done` de `updateSprintStatus` (AC-3.1.1): todo REQ `included` atribuído à sprint exige todos os AC ligados `proved` — um irmão unproved bloqueia, sem write; **INV3 nos dois sentidos** — "sprint marcada sem ledger" E "ledger marcado sem sprint" bloqueiam o fechamento com `alinhar_marcadores_traceability`; ledger ilegível na sprint v1 vira pendência dedicada (`corrigir_ledger_traceability`); sprint legacy intocada. **Wire de produção no `talos_verify_sprint_file`**: quando o caller informa `backlog_path`, o ledger alimenta o ramo v1 do conformance público (refs inválidas/órfãs e mismatch de marcas bloqueiam antes do selo/execução) — antes o grafo só rodava sob injeção nos testes.
- **`packages/skills/_shared/scripts/document_quality.mjs`** — `applyItemField` mapeia `source_refs` (antes no-op); `traceabilityMode` (v1/legacy/inconsistent por par metadado↔ledger); `checkTraceabilityGraph` (refs válidas/órfãs, included sem AC, N:N com motivo; ref duplicada no mesmo AC não fabrica N:N); ramo v1 no conformance.
- **`packages/templates/SPRINT_TEMPLATE.md`** — metadado `Traceability` (default legacy) + exemplo `source_refs` parseável nos ACs exemplificados (sem comentário inline — o parser lê apenas a lista).
- **`packages/orchestrator/skills/talos/SKILL.md`** — gate de fechamento (SPRINT_STATUS_SYNC) menciona o gate v1, o registro do par via `upsert` no momento da marcação, `record_metric` pós-execução e o receipt ecoado da action `receipt` (sem recalcular cobertura).
...
Validação:
- `node --test packages/mcp-server/server.test.js build/tests/etapa3.test.mjs` — 360/360 (inclui lado B do INV3 nos gates, escopo de sprint do receipt, ref duplicada sem N:N falso e exemplo do template parseável).
...
- **`AGENTS.md`** — "Oito hosts" → "Nove hosts" com parágrafo sobre MinimaxCode (Plugin V1 + 5 custom agents + `TALOS_HOST=mavis`).

Patch subsequente (Unreleased → `0.18.2`):
- **`build/install-host.sh` (mavis)** — corrige o motivo pelo qual o Plugin V1 não era reconhecido pelo runtime do MinimaxCode mesmo com o manifest e os 10 skills no disco. Causa raiz: o `servers.mcp.json` referenciava o `server.js` por **path absoluto** (`$ROOT/packages/mcp-server/server.js`), e o reader de Plugin V1 do MinimaxCode (`@mavis/local-runtime-v2/dist/service/plugin-system/mcp/config.js:normalizeStdioTransport`, modo `official`) **rejeita** args com path absoluto (`readerFail('MCP_SCHEMA_INVALID', 'contains an absolute stdio argument')`). Como o `minimax-reader` sempre delega o MCP para `readOfficialMcpFile` (mesmo com `source: 'LOCAL_MINIMAX'`), o plugin inteiro falhava a validação → não era registrado em `local_runtime_plugin_official_state` → 10 skills não carregavam. O `mcp` `talos` só funcionava porque era registrado na unha via `mavis mcp create` (workaround que vira dívida técnica). Fix: (1) **bundlear** o `server.js` dentro do Plugin V1 em `~/.minimax/plugins/talos/server.js` e usar `args: ["./server.js"]` (relativo, dentro do package); (2) **bundlear a árvore `skills/_shared/`** que o `server.js` importa via path relativo hard-coded `../skills/_shared/scripts/document_quality.mjs` — o destino precisa ser sibling do `talos/` (`~/.minimax/plugins/skills/_shared/...`) porque o path `../skills/_shared/` é relativo ao `server.js` e sai do package root; o loader de plugins do MinimaxCode ignora subdiretórios sem manifest, então o sibling é inerte pro scan. Sem essa estrutura o MCP também falhava com `ERR_MODULE_NOT_FOUND` no boot. Sem mudança no motor, schema MCP, gates, disco v3 ou topologia.
...
- Quem instalar MinimaxCode ganha um nono host: packager único (`build/install-host.sh mavis`) materializa Plugin V1 + 5 custom agents em `~/.minimax/`; re-scan do MinimaxCode descobre o plugin automaticamente; `talos_ping` confirma boot; `talos_capabilities` lista `mavis` em `known_hosts`.
...
Validação:
- `node build/smoke-hosts.mjs` — ok (9 hosts × boot + detecção + capabilities + ping, incluindo MinimaxCode).
- Bump `VERSION` + release do `0.18.2` pendente de validação end-to-end no MinimaxCode (instalação real + dispatch de subagente + preflight reportando `dispatch_mutable: true`).
