...
Smoke pós-update: `talos_ping` → `version: 0.15.2`; `talos_capabilities` → `schema_version: 5` (sibling-only).
...
- `talos_ping` → deve retornar `host=<claude|codex|antigravity|zcode|opencode|pi|vscode>` e `version: 0.15.2`
