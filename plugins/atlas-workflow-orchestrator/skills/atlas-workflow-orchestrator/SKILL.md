...
- **v0.7.2** — Patch de confiabilidade (sem breaking, schema v5 intacto). `ping().capabilities` passa a ser **derivado de `toolsList()`** (fonte única) — fim do drift que mantinha lista manual paralela e omitia `atlas_classify_input`, capaz de travar run válida na Fase 0; guard cruzado novo em `server.test.js`. CI ganha job `cross-os` (Windows/macOS, núcleo MCP + smoke + conformance em Node puro). Doc: nota de proveniência do `dispatch_token` no `atlas-task-validator` (quem lê `validator_recovery` é o validador irmão, não o orquestrador) e `.gitattributes` marcando artefatos gerados (`hosts/`, `plugins/`, `.agents/`, `archive/`, `dist/`).
- **v0.7.1** — Patch de confiabilidade (sem breaking, schema v5 intacto). Correções no MCP: `atlas_run_state(upsert)` faz merge top-level (não derruba `dispatch.active` em upsert parcial — bug que travava o `lock_validator(start)` seguinte); `findActiveRunConflict` só bloqueia conflito de lock real (run inativo/de versão anterior deixa de travar runs novos); `atlas_verify_artifact` aceita `artifact_kind` para banner correto de PRD. Gate G4 endurecido: falha de dispatch do validador em runtime = `blocked`, nunca inline (R17); `dispatch_token` do `complete` tem que vir do output do próprio validador irmão (R19). Limite honesto: não é prova de isolamento não-forjável (MCP não distingue caller no stdio único) — fecha o atalho preguiçoso, prova forte fica para sprint futura.
