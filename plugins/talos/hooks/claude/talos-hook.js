...
function block(message, details = {}) {
  ...
  process.stderr.write(`Talos hook blocked.\n${message}${suffix}\n`);
  process.exit(BLOCK);
}
...
function readLatestState(cwd) {
  ...
  try {
    ...
    block('Nao foi possivel ler .talos/state. Estado nao comprovado; hook nao aprova implicitamente.');
  }
  ...
  for (const file of files) {
    try {
      ...
      block('Nao foi possivel inspecionar estado .talos/state. Estado nao comprovado; hook nao aprova implicitamente.', { file });
    }
  }
  ...
  try {
    ...
    block('Estado .talos/state corrompido ou ilegivel. Proxima acao: recuperar o estado MCP antes de continuar.', { file: latest.file });
  }
}
