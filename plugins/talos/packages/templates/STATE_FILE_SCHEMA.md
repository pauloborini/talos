...
- `base_sha` e `head_sha` são commits explícitos; não inferir base pelo nome da branch. `HEAD` atual pode avançar se `head_sha` for ancestral e `git diff head_sha HEAD` nos paths da slice for vazio (commit paralelo fora da fatia). Drift ou rewrite nesses paths, ou `head_sha` fora da história, invalida o boundary. `files_changed` e os arquivos de `task_evidence`/`repair_evidence` devem cobrir o diff `base_sha...head_sha` somado ao delta `worktree_baseline→worktree_final` **restrito aos paths da slice** (declarados + evidência). Dirty preexistente byte/status-idêntico fica fora. WIP paralelo fora da slice permanece no snapshot completo, não entra em `files_changed` e **não** invalida `worktree_final` nos gates G4/G12.
