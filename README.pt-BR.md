...
- **`talos_select_next_sprint` matura stub sem `--loop`.** Fila `backlog` com stub válido retorna `next_action: sprint_interview` antes de `ready`.
- **PLAN ⊆ §2 via `intent_refs`.** Cada task declara `SF-*`/`R1` da §2; `talos_assert_after_plan` valida cobertura mecânica quando `require_sprint_file`.
- **Sem `legacy_sealed`.** Sprint aberta em `doing`/`review` migra as linhas de intenção da §1 (template 0.23) e completa L2 antes de plano/direct. Sem `Intenção status` / `Selo da intenção` falha até `require: stub`.
...
|------|-------------|-----------|
| **`full`** | Sprint/backlog novo ou feature do zero | Satura intenção §2, matura contrato §7 (entrevista se preciso) → **plano** (`.talos/plans/`) → **executa** o plano → review (opcional ou obrigatória via `critical_review`) |
| **`direct`** | Intenção §2 e contrato §7 já aprovados+selo | Valida sprint/§2/§7 → entrevista só se houver gap → **executa direto** (sem fase de plan handoff) → review (opcional ou obrigatória via `critical_review`) |
...
| `interview-only` | Só fechar decisões / brainstorm | Entrevista intenção §2 e depois o contrato §7; não implementa |
...
**Dica:** `full` = “quero intenção §2 + contrato §7 + plano + código”. `direct` = “já tenho §2 saturada e §7 aprovado, implementa”. `execute` = “já tenho o plano, só executa”. `audit` = “diagnostica, não corrige”.
